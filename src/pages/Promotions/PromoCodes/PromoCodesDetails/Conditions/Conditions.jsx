...
const Conditions = () => {
    ...
    const { api } = authCtx;
    const { pid: campaignId } = useParams();
                    ...
                    if (t_row?.type) {
                        if (t_row.type === "group") {
                            ...
                            switch (t_row?.type_component) {
                                case "main":
                                    return <MainGroup key={t_row.id} id={t_row.id} data={t_row} rules={rules} handleAddComponent={handleAddComponent} handleRemoveComponent={handleRemoveComponent} />;
                                case "customer":
                                    return (
                                        <CustomerGroup key={t_row.id} id={t_row.id} data={t_row} rules={rules} handleAddComponent={handleAddComponent} handleRemoveComponent={handleRemoveComponent} />
                                    );
                                case "cart_summary":
                                    return (
                                        <CartSummaryGroup
                            ...
                            switch (t_row?.type_component) {
                                case "customer":
                                    return <CustomerRow key={t_row.id} id={t_row.id} data={t_row} handleRemoveComponent={handleRemoveComponent} campaignId={campaignId} />;
                                    break;
                                case "cart_summary":
                                    return <CartSummaryRow key={t_row.id} id={t_row.id} data={t_row} handleRemoveComponent={handleRemoveComponent} campaignId={campaignId} />;
                                    break;
                                case "default":
                                default:
                                    return <Row key={t_row.id} id={t_row.id} data={t_row} handleRemoveComponent={handleRemoveComponent} campaignId={campaignId} />;
                            }
                        }
                    } else {
    ...
    const checkIfAllSelected = (parent) => {
        for (const rule of parent.rules) {
            if (rule.type === "row") {
                ...
                if (!checkIfAllSelected(rule)) return false;
            }
        }
        return true;
    };
