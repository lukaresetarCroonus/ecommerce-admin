...
const CalculateForm = ({ campaignId }) => {
  ...
  const { nid } = useParams();
  const apiPath = "admin/campaigns/cart-delivery/calculations";
  ...
  const chageHandler = (data, fieldName) => {
    api.get(`admin/campaigns/cart-delivery/calculations/ddl/currency?discount_type=${data.discount_type}`)
      .then((response) => {
